...
void toggle_overhang_toplevel(struct area_container *container, tiley::WindowStateManager& manager, tiley::TileyServer& server, int workspace){
    ...
    wlr_box display_box = get_display_geometry_box(server);
    ...
    if(container->floating == NONE){
        ...
        output_box = get_target_geometry_box(target_container, display_box.width, display_box.height);
        split_info split = output_box.width > output_box.height ? SPLIT_H : SPLIT_V;
