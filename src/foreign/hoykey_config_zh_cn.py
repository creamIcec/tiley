...
IMPORTANT_ACTIONS = {
    ...
    "launch_app_launcher": "启动应用启动器",
    "close_window": "关闭当前窗口",
    "quit_compositor": "退出 Tiley (注销)",
    "lock_screen": "锁定屏幕",
    "toggle_floating": "切换窗口浮动/平铺",
    "fullscreen_toggle": "切换窗口全屏",
...
class KeybindingEditor:
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(main_frame, text="Tiley 核心快捷键", font=("Helvetica", 18, "bold"))
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20), sticky="w")
        
        help_label = ttk.Label(main_frame, text="点击输入框开始录制快捷键，按 ESC 取消", 
        ...
        for action, description in IMPORTANT_ACTIONS.items():
            ...
            current_key = self.all_bindings.get(action, "尚未绑定")
        ...
        save_button = ttk.Button(button_frame, text="保存并应用", command=self.on_save, style="Accent.TButton")
    ...
    def stop_recording(self, restore_original=False):
        ...
        if restore_original:
            original_value = self.all_bindings.get(action, "尚未绑定")
    ...
    def notify_tiley(self):
        try:
            ...
            if result.returncode == 0:
                messagebox.showinfo("应用成功", "Tiley 已收到通知并重新加载了快捷键！")
            else:
                messagebox.showwarning("提醒", "未找到正在运行的 Tiley 进程。\n配置已保存，下次启动 Tiley 时将生效。")
        except Exception as e:
            messagebox.showerror("错误", f"通知 Tiley 失败: {e}")
